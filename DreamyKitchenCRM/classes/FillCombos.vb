...
Public Class FillCombos
    ...
    Public Sub STATUS(CtrlCombo As DevExpress.XtraEditors.LookUpEdit)
        Try
            Dim cmd As SqlCommand = New SqlCommand("Select id,Name from vw_STATUS order by name", CNDB)
            Dim sdr As SqlDataReader = cmd.ExecuteReader()
    ...
    Public Sub SALERS(CtrlCombo As DevExpress.XtraEditors.LookUpEdit)
        Try
            Dim cmd As SqlCommand = New SqlCommand("Select id,Name from vw_SALERS order by name", CNDB)
            Dim sdr As SqlDataReader = cmd.ExecuteReader()
